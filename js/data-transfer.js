...
/**
 * Verificar e exibir informações sobre backups automáticos
 */
function getBackupInfo() {
    ...
    for (let i = 0; i < localStorage.length; i++) {
        ...
        if (key && key.startsWith('lifestyleData_backup_')) {
            const timestamp = parseInt(key.replace('lifestyleData_backup_', ''));
...
function getDataStatistics() {
    ...
    scheduleKeys.forEach(key => {
        ...
        if (schedule.isPlanned) stats.plannedSchedules++;
        if (key === getDateKey()) stats.todaySchedules++;

        if (schedule.activities) {
            stats.totalActivities += schedule.activities.length;

            schedule.activities.forEach(activity => {
                if (activity.type === 'work') stats.categoryCounts.work++;
                if (activity.type === 'study') stats.categoryCounts.study++;
                if (activity.type === 'cleaning') stats.categoryCounts.cleaning++;
            });
        }
    });
